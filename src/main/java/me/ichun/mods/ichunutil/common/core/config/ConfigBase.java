...
public abstract class ConfigBase implements Comparable<ConfigBase>, IIdentifiable, IListable
{
    ...
    public void readProperty(Field field, boolean write)
    {
        ...
        String comment = propInfo.comment().equals("undefined") ? new TextComponentTranslation(getModId().toLowerCase() + ".config.prop." + field.getName() + ".comment").getFormattedText() : new TextComponentTranslation(propInfo.comment()).getFormattedText();
        ...
        String commentLocal = new TextComponentTranslation(comment).getFormattedText();
        ...
        try
        {
            ...
            PropInfo propInfo1 = new PropInfo(!propInfo.nameOverride().isEmpty() ? propInfo.nameOverride() : new TextComponentTranslation(getModId().toLowerCase() + ".config.prop." + field.getName() + ".name").getFormattedText(), comment, field);
    ...
    public void setCategoryComments()
    {
        for(Map.Entry<CategoryInfo, ArrayList<ConfigBase.PropInfo>> e : categories.entrySet())
        {
            ...
            {
                info.name = new TextComponentTranslation(String.format("ichunutil.config.cat.%s.name", cat)).getFormattedText();
                comment = new TextComponentTranslation(String.format("ichunutil.config.cat.%s.comment", cat)).getFormattedText();
            }
            else
            {
                info.name = new TextComponentTranslation(getModId().toLowerCase() + ".config.cat." + cat + ".name").getFormattedText();
                comment = new TextComponentTranslation(getModId().toLowerCase() + ".config.cat." + cat + ".comment").getFormattedText();
            }

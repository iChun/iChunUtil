...
public class ModelHelper
{
    ...
    public static HashMap<String, ModelRenderer> getModelCubesWithNames(ModelBase parent)
    {
        ...
        {
            list.entrySet().removeIf(e -> e.getValue() == child);
        }
            ...
            try
            {
                ...
                while(clz != Render.class)
                {
                    ...
                    if(clz == RenderLivingBase.class)
                    {
                        ...
                        {
                            f.setAccessible(true);
//                            if(ModelBase.class.isAssignableFrom(f.getType()) && (f.getName().equalsIgnoreCase(ObfHelper.mainModel[0]) || f.getName().equalsIgnoreCase(ObfHelper.mainModel[1]))) //FIXME is public, but not static
                            {
                                ModelBase base = (ModelBase)f.get(rend);
                                if(base != null)
                                {
    ...
    {
        ArrayList<ModelRenderer> map = classToModelRendererMap.computeIfAbsent(rend.getClass(), k -> getModelCubes(getPossibleModel(rend)));
        return map;
    }
